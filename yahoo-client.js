...
const YahooClient = (function () {
  ...
  async function fetchWithRetry(url, retries = 3) {
    ...
    let startIdx = proxyKeys.indexOf(currentProxy);
    if (startIdx < 0) startIdx = 0;

    for (let attempt = 0; attempt <= retries; attempt++) {
      const proxyKey = proxyKeys[(startIdx + attempt) % proxyKeys.length];
      currentProxy = proxyKey;

      // Update dropdown
      const select = document.getElementById('proxySelect');
      if (select) select.value = proxyKey;
      ...
      try {
        ...
        // Different proxies encode differently
        if (proxyKey === 'codetabs') {
          fullUrl = proxy + encodeURIComponent(url);
        } else if (proxyKey === 'thingproxy') {
          fullUrl = proxy + url;
        } else {
          fullUrl = proxy + encodeURIComponent(url);
        }
        ...
        if (!response.ok) {
          ...
          // Check if it's a CORS proxy error page
          if (text.includes('cors') || text.includes('blocked') || text.includes('forbidden')) {
            throw new Error(`Proxy ${proxyKey} blocked: ${response.status}`);
          }
        ...
        console.warn(`Proxy ${proxyKey} failed (attempt ${attempt + 1}):`, err.message);

        if (attempt === retries) {
          throw new Error(
            `All proxies failed. This usually means:
` +
            `1. Yahoo Finance is temporarily blocking requests
` +
            `2. All CORS proxies are down or rate-limited
` +
            `3. Your network blocks certain proxies
...
` +
            `• Using a VPN
` +
            `• Clicking "Load Demo Data" to see the UI`
          );
        }
  ...
  async function fetchQuote(ticker) {
    ...
    return {
      ...
      change: (meta.regularMarketPrice || meta.previousClose || 0) - (meta.previousClose || meta.chartPreviousClose || 0),
